...
[https://your-github-repo-url](https://github.com/mwango2027/Wang_Maddie_201_A06.git)
...
```{r}
# initializing the list of heights
height <- list(78, 81, 78, 86, 85)

# looping it to add the respective height to each player's list
...
# looping to fill the list with each player's points per game (ppg)
for (i in 1:length(player_info)){
  ppg <- player_info[[i]]$points/player_info[[i]]$games_played
  ppg_list <- append(ppg_list, ppg)
}

# renaming each ppg element to respective player's initials
names(ppg_list) <- c('MJ', 'LJ', 'KB', 'KA', 'SO')
...
```{r}
# creating function to round value to 2 digits
round_2 <- function(x){
  return(round(x, digits = 2))
}

# applying to list with sapply for a vector output
rounded_ppgs <- sapply(ppg_list, round_2)
rounded_ppgs
```
...
# rounding to either tenths, hundredths, or thousandths
round_specific <- function(x){
  if (x < 100){
    return(round(x, -1))
  }else if (x < 1000){
    return(round(x, -2))
...
# applying to matrix
mat_rand <- apply(mat_rand, MARGIN = 1:2, round_specific)
mat_rand
...
```{r}
# initializing column title list to use for indexes
col_titles <- c("jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec")

# creating a list and using sapply with a function to calculate means per each column
avg_list <- sapply(col_titles, function(col){mean(max_temps[[col]])})
avg_list
...
```{r}
# slicing for rows that fit the desired state initial logical
...
# creating a list of median annual temperatures
median_list <- apply(co[,3:14], 1, median)

# creating a list of all the years to use for indexing
years <- co$year

# finding maxes
max_med <- median_list[[which.max(median_list)]]
max_med_year <- years[[which.max(median_list)]]

# finding mins
min_med <- median_list[[which.min(median_list)]]
min_med_year <- years[[which.min(median_list)]]

# pasting results
paste0("The maximum median annual temperature of ", max_med, " occured in ", max_med_year, " and the minimum median annual temperature of ", min_med," occured in ", min_med_year, ".")
```
